...
@Injectable()
export class SummaryBillboardService<T> extends CardPluginBaseClassService<T> implements DashboardCardDataGridReadable<T> {
    ...
    streamInitialDataModel(dcrf: DashboardConfigurationResourceFacade<DashboardConfigurationResourceCardSchema, DashboardConfigurationResourceCardSchema>,
        ...
        // process the virgo entity, expected poor performance
        const o$ = this._templateTransporter.awaitingResourceStream(serviceResourceFacade).pipe(
            map((d: boolean) => {
                const entity: SummaryBillboardDataModelSchema = serviceResourceFacade.resolver.getSnapshotData();
